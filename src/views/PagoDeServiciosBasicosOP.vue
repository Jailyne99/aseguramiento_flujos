      ...
      <v-col md="8">
        <v-card elevation="2">
          <v-container class="background" fluid>
            <h3>Pago de servicos basicos</h3>
          </v-container>
          <div
            id="myDiagramDiv"
      ...
      <v-col md="4" >
        <v-card class="pb-2" elevation="2">
          ...
          <v-alert class="ma-4" outlined type="success" text>
            ...
            adquisición de servicios y justificación de bienes y/o servicios ..."
             Se contaba con 16 hrs, pero por temas de urgencia de información,
             se cambió a 2hr, 1hr si surgen inconvenientes.
          </v-alert>
          <v-alert class="ma-4 mb-2" outlined type="success" text>
            En el proceso "Recepcionar el expediente completo para pago"
            Se contana con 8 horas establecidas, pero se verificó el tiempo 
            necesario para llevar a revisión los expedientes, para que se 
            cumpla los requerimientos necesarios.
          </v-alert>
        </v-card>
      </v-col>
    </v-row>
  ...
  methods: {
    async loadData() {
      ...
      const response = await fetch(
        `http://localhost:5010/api/procedimientos/mejorada/op/${3}`
      );
      this.itemsTask = await response.json();
      console.log("Este es el id: " + this.itemsTask[0].id);
      const response2 = await fetch(`http://localhost:5010/api/enlace/${3}`);
    ...
    async selectorItem(id) {
      ...
      if (this.codigo != null) {
        ...
        const response = await fetch(
          `http://localhost:5010/api/tareas/op/${3}`
        );
        this.itemsTask = await response.json();
